...
export const generateFinancialReport = async (
    ...
    const expenses = transactions
        .filter((t) =>
            ["LOAN_DISBURSEMENT", "SAVINGS_WITHDRAWAL"].includes(t.type)
        )
        .reduce((sum, t) => sum + Number(t.amount), 0);
    ...
    const memberSummary = includeMemberDetails
        ? transactions
              ...
              .reduce((acc, t) => {
                  ...
                  const memberName = `${t.member!.first_name} ${
                      t.member!.last_name
                  }`;
...
export const generateLoanRepaymentReport = async (
    ...
    const byRank = Object.entries(
        repayments.reduce((acc, r) => {
            const rank = r.loan.member.Personel?.rank || "CIVILIAN";
            acc[rank] = acc[rank] || { due: 0, paid: 0, overdue: 0 };
            acc[rank].due += Number(r.amount);
            if (r.status === "PAID") acc[rank].paid += Number(r.amount);
            if (r.status === "LATE" || r.status === "DEFAULTED")
                acc[rank].overdue += Number(r.amount);
            return acc;
        }, {} as Record<string, { due: number; paid: number; overdue: number }>)
    ).map(([rank, stats]) => ({ rank, ...stats }));
    ...
    const memberDetails = includeMemberDetails
        ? repayments.map((r) => ({
              ...
              member: {
                  ...
                  type: r.loan.member.type,
                  rank: r.loan.member.Personel?.rank || null,
                  unit: r.loan.member.Personel?.unit || null,
              },
          }))
        : undefined;
...
export const generateMemberFinancialSummary = async (
    ...
    const report = {
        ...
        recentTransactions: transactions.slice(0, 10).map((t) => ({
            ...
            savingsCategory: t.savings?.category.name || null,
        })),
    };
...
export const getAllMembersFinancialSummary = async (
    ...
    const memberData = members.map((member) => {
        ...
        return {
            ...
            activeLoanCount: memberLoans.filter((l) => l.status === "ACTIVE")
                .length,
    ...
    const report = {
        ...
        summary: {
            ...
            totalSavings: savings.length,
            totalSavingsAmount: savings.reduce(
                (sum, s) => sum + Number(s.amount),
                0
            ),
        },
